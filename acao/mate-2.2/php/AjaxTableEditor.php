...
class AjaxTableEditor
{
	...
	function displayTableHtml()
	{
		...
		$numRows = 0;
		$this->extraOrderByInfo = empty($this->extraOrderByInfo) ? '' : ', '.$this->extraOrderByInfo;
		// Added tick marks using implod/explode in case custom  by columns have periods in them.
	...
	{
		$html = '<select id="'.$colName.'" name="'.$colName.'" '.$otherInfo.'>';
		$result = $this->doQuery($query);
	...
	{
		$html = '<select id="'.$nameAndId.'" name="'.$nameAndId.'" '.$otherInfo.'>';
